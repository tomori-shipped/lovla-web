...
import { activitiesPost as post, activityGroups } from "@/constants/blog/long-distance-activities";
  ...
  "@graph": [
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { "@type": "ListItem", position: 3, name: "Long-distance relationship activities", item: canonical },
      ],
    },
  ],
};
      ...
      <main>
        <article>
          <header className="mx-auto max-w-4xl px-6 pb-10 pt-12 text-center sm:px-8 md:pb-12 md:pt-20">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#A93F7B]">Long-distance, a little closer</p>
          ...
          <div className="mx-auto grid max-w-6xl gap-10 px-6 py-12 sm:px-8 md:py-16 lg:grid-cols-[210px_minmax(0,1fr)] lg:gap-14">
            <aside>
              <nav aria-label="In this guide" className="rounded-xl border border-primary/10 p-5 lg:sticky lg:top-8 lg:rounded-none lg:border-0 lg:p-0">
                <p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#786577]">Find your kind of date</p>
                <ul className="mt-4 space-y-3">
                  {activityGroups.map((group) => <li key={group.id}><a href={`#${group.id}`} className="block py-1 text-sm leading-6 text-primary underline-offset-4 hover:underline">{group.shortTitle}</a></li>)}
                </ul>
              </nav>
            </aside>
            <div className="min-w-0 max-w-[740px] text-[17px] leading-[1.85] text-[#4F4350]">
              <div className="space-y-5">
                <p>There’s a particular kind of long-distance evening where you’ve both said “so, what else?” twice, and neither of you wants to hang up. You miss your person. You just don’t have another hour of updates in you.</p>
                <p>That’s where having something to <em>do</em> helps. Long-distance relationship activities can be as small as a five-minute photo game or as involved as cooking dinner together. The useful ones fit the time, energy, and money you actually have.</p>
                <p>Below are 45 ideas, grouped by what your evening looks like. Many cost nothing beyond supplies you already own; some involve an app, ingredients, or postage. Start with one, not a whole new relationship routine.</p>
              </div>
              <div className="my-9 border-l-2 border-[#D98AB8] py-1 pl-5">
                <p className="font-semibold text-primary">Can’t decide? Try this tonight.</p>
                <p className="mt-2">Each bring three photos from your day. Take turns explaining them, then choose one longer idea for another evening. Ten minutes is enough to start.</p>
              </div>
              {activityGroups.map((group) => (
                <section key={group.id} id={group.id} aria-labelledby={`${group.id}-title`} className="scroll-mt-8 border-t border-primary/10 pt-10 mt-12">
                  <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#A93F7B]">{group.timing}</p>
                  ...
                  <ol start={group.start} className="mt-8 ml-7 list-decimal space-y-8 marker:font-semibold marker:text-primary">
                    {group.activities.map((activity) => (
                      <li key={activity.title} className="pl-1 sm:pl-2">
                        <h3 className="text-xl font-semibold leading-snug tracking-[-0.015em] text-primary">{activity.title}</h3>
              ...
              <section aria-labelledby="make-it-work" className="mt-12 border-t border-primary/10 pt-10">
                <h2 id="make-it-work" className="font-helix text-3xl leading-tight tracking-[-0.02em] text-primary sm:text-4xl">A date shouldn’t become another task</h2>
                <p className="mt-5">Before you pick an activity, ask three things: how much time do we have, how tired are we, and do we want to talk or make something? The answer might be different for each of you. Choose the smaller plan if that makes it easier.</p>
                <p className="mt-5">Agree on a rough finish time, especially across time zones. Take turns suggesting ideas, and keep a backup for bad internet or low energy. A short voice note can stand in for a video date without anybody having to apologize for it.</p>
                <p className="mt-5">You don’t need to work through all 45 activities. Find two or three you both genuinely enjoy and return to them. Having a familiar little ritual can be lovely precisely because you don’t have to plan it again.</p>
              </section>
              <section aria-labelledby="lovla-date" className="my-12 rounded-2xl bg-[#F0E6F0] p-6 sm:p-8">
                <p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#8C4677]">One idea to make your own</p>
                <h2 id="lovla-date" className="mt-3 font-helix text-3xl leading-tight text-primary">Your favorite photo. Your next date.</h2>
                <p className="mt-4">Lovla turns your photos into coloring pages you can work on together in real time, then save in a shared journal. Choose a memory and give yourselves a little time to revisit it.</p>

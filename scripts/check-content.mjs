...
const facetimeSlug = "/blog/things-to-do-on-facetime-with-your-partner";
const questionsSlug = "/blog/who-knows-me-better-questions-for-couples";
const routes = ["/", "/blog", slug, questionsSlug, facetimeSlug, "/blog/best-apps-for-couples", "/contact-us", "/privacy-policy", "/terms-and-conditions", "/sitemap.xml", "/robots.txt"];
...
assert(!home.includes("android-waitlist"), "Retired waitlist is absent from the homepage");
assert(pages.get("/blog").includes(`href="${facetimeSlug}"`), "Blog index retains the FaceTime guide");
assert(pages.get("/sitemap.xml").includes(`https://www.lovla.app${facetimeSlug}`), "Sitemap retains the FaceTime guide");
